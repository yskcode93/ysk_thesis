...
from data_for_cv import CustomDataset, count_nonzero_elements_per_row
...
def final_saksub_convert(gpu, world_size, ConverterClass, config, weight_path, n_epochs, batch_size, lr, warmup, use_apex, finetune,\
    ...
    X_train = np.load(train_src_path)
    
    train_tgt_path = "./Final_Data/sak_to_sub/npy/tgt_train.npy"
    Y_train = np.load(train_tgt_path)
    
    #trainにおけるsrcの配列長+eosの長さを獲得
    train_non_target = count_nonzero_elements_per_row(X_train)
    ...
    X_valid = np.load(valid_src_path)
    
    valid_tgt_path = "./Final_Data/sak_to_sub/npy/tgt_valid.npy"
    Y_valid = np.load(valid_tgt_path)
    
    #validにおけるsrcの配列長+eosの長さを獲得
    valid_non_target = count_nonzero_elements_per_row(X_valid)
    ...
    #カスタムデータセット版
    train_dataset = CustomDataset(X_train, Y_train, train_non_target)
    valid_dataset = CustomDataset(X_valid, Y_valid, valid_non_target)
    ...
    for i in range(1, n_epochs+1):
        ...
        for x,y, non_tgt_len in train_loader:
            optimizer.zero_grad()
            x, y, non_tgt_len = x.cuda(non_blocking=True), y.cuda(non_blocking=True), non_tgt_len.cuda(non_blocking=True)

            out = model(x, mask)
            #出力は(batch_size, seq_len, vocab_size)
            out = out[:, non_tgt_len+2 : non_tgt_len+2+701, :]
        ...
        total_valid_loss = 0
        with torch.no_grad():
            for x,y, non_tgt_len in valid_loader:
                x, y, non_tgt_len = x.cuda(non_blocking=True), y.cuda(non_blocking=True), non_tgt_len.cuda(non_blocking=True)
...
def final_m2m_convert_valid(gpu, world_size, ConverterClass, config, weight_path, n_epochs, batch_size, lr, warmup, use_apex, finetune,\
    ...
    src_tagpath = "./Final_Data/convert_data/numpy/src_all_8_tag.npy" 
    src_tag = np.load(src_tagpath)
    
    tgt_tagpath = "./Final_Data/convert_data/numpy/tgt_all_8_tag.npy" 
    tgt_tag = np.load(tgt_tagpath)
    
    src_path = "./Final_Data/convert_data/numpy/small/src_all_8.npy" 
    X = np.load(src_path)
    
    tgt_path = "./Final_Data/convert_data/numpy/small/tgt_all_8.npy"
    Y = np.load(tgt_path)
    
    #validにおけるsrcの配列長+eosの長さを獲得
    non_target = count_nonzero_elements_per_row(X)
    ...
    #カスタムデータセット版
    dataset = CustomDataset(X, Y, non_target)
