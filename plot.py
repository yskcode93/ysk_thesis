...
for url in df['RefSeq FTP']:
    ...
    download_folder = './Data/amino'
    command = f'curl -fsSL {url_faa} | gzip -d > {os.path.join(download_folder, os.path.basename(url) + "_translated_cds.faa")}'
    #command = f'curl -fsSL -O {url_faa}'
    #command = f'curl {url_faa} | tar xvf - -C /Data/amino {url_fna} | tar xvf - -C /Data/codon'
...
download_folder ="./Data/codon"  # ダウンロード先フォルダのパスを指定
...
for url in df['RefSeq FTP']:
    ...
    if os.path.exists(os.path.basename(os.path.basename(url) + '_cds_from_genomic.fna.gz')):
         continue
    #if os.path.exists(os.path.join(download_folder, os.path.basename(url) + '_translated_cds.faa')):
    #   continue
    
    print("downloading", url_fna)
    #print("downloading", url_faa)
    file_path = os.path.join(download_folder, os.path.basename(url) + '_cds_from_genomic.fna.gz')
    #file_path = os.path.join(download_folder, os.path.basename(url) + '_translated_cds.faa.gz')
    urllib.request.urlretrieve(url_fna, file_path)
    with gzip.open(file_path, 'rb') as f_in, open(os.path.join(download_folder, os.path.basename(url) + '_cds_from_genomic.fna'), 'wb') as f_out:
