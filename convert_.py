...
def final_converter(gpu, world_size, ConverterClass, weight_path, n_epochs, batch_size, lr, warmup, use_apex, finetune,\
    ...
    # バリデーションデータのパスを指定
    valid_srcpath = "./Data/sak_to_sub/valid/src_valid.npy"
    valid_tgtpath = "./Data/sak_to_sub/valid/tgt_valid.npy"
    
    valid_X = np.load(valid_thupath)
    valid_Y = np.load(valid_subpath)
    valid_Y_sp = torch.zeros(valid_X.shape[0], 1, dtype=torch.int64)
    
    # バリデーションデータのデータセットを作成
    valid_dataset = dat.TensorDataset(
        torch.from_numpy(valid_X.astype(np.int64)),
        torch.from_numpy(valid_Y.astype(np.int64)),
        valid_Y_sp
    )

    # バリデーションデータ用のデータローダーを作成
    valid_loader = dat.DataLoader(
        dataset=valid_dataset,
        batch_size=batch_size,  # バッチサイズはトレーニングと同じか異なる値を指定
        shuffle=False,
        num_workers=0,
        pin_memory=True
    )
    ...
    best_valid_loss = float('inf')  # 最良のバリデーションロスを追跡
    early_stopping_patience = 5  # 早期終了の許容エポック数
    last_best_epoch = 0  # 最後に最適なモデルが保存されたエポック
    ...
    for i in range(1, n_epochs+1):
        ...
        if rank == 0 and i%log_interval==0:
            with open("{}/for_8_8.log".format(output_dir), "a") as f:
                f.write("EPOCH({:0=3}%) TRAIN LOSS: {:.4f} lr={:.4f} x 1e-4 ELAPSED TIME: {:.4f}s\n".format(int(i*100/n_epochs), avg_loss,\
                    scheduler.get_last_lr()[0]*1e4, time.time()-start))
        
        # バリデーションデータでのループ
        if valid_loader is not None and i%log_interval==0:
            model.eval()
            valid_loss = 0
            with torch.no_grad():
                for x_val, y_val, y_sp_val in valid_loader:
                    x_val, y_val, y_sp_val = x_val.cuda(non_blocking=True), y_val.cuda(non_blocking=True), y_sp_val.cuda(non_blocking=True)
                    if pretrain:
                        with torch.no_grad():
                            x_e_val = model_pretrain_en.get_output(x_val)
                        x_e_val = torch.transpose(x_e_val, 1, 2)
                        out_val = model(x_e_val, x_val[:,1:], y_sp_val, y_val[:,1:]) if pretrain and "n_species" in config_pretrain else model(x_e_val, x_val, y_sp_val, y_val)
                        out_val = torch.transpose(out_val, 1, 2)
                    else:
                        x_val = torch.tensor(x_val, dtype=torch.float32)
                        out_val = model(x_val, x_val, y_sp_val, y_val)
                        out_val = torch.transpose(out_val, 1, 2)
                    loss_val = criterion(out_val.permute(0,2,1), y_val[:, 1:] if pretrain and "n_species" in config_pretrain else y_val)
                    valid_loss += loss_val.item()
            valid_loss /= len(valid_loader)

            if rank == 0:
                with open("{}/for_8_8.log".format(output_dir), "a") as f:
                    f.write("EPOCH({:0=3}%) VALID LOSS: {:.4f}\n".format(int(i*100/n_epochs), valid_loss))

            # 最適なモデルを保存（validの損失が改善した場合）
            if rank == 0 and (i==0 or i%save_interval==0):
                if valid_loss < best_valid_loss:
                    ...
                    torch.save(checkpoint, "{}/weight/weight_M2_8_8/sak_sub_3_{}.pt".format(output_dir, i))
                    last_best_epoch = i
                else:
                    # validの損失が改善しない場合、学習を早期終了
                    if early_stopping_patience > 0 and i - last_best_epoch >= early_stopping_patience:
                        break
